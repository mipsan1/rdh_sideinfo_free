"""
Step 4: median-of-4 predictor (recoverable, dot-only) + windowed-variance sorting
+ double-layer shifting PEE. Map-free, fully reversible.
Predictor swapped from mean4 -> median4 (sharper PEH => higher e0 => higher PSNR).
"""
...
def context_complexity(img,parity,radius=2):
    ...
    other=(grid!=parity).astype(np.float64)
    val=img.astype(np.float64)*other; val2=(img.astype(np.float64)**2)*other
    ic=integral(other); iv=integral(val); iv2=integral(val2)
    cnt=box_sum(ic,radius); s1=box_sum(iv,radius); s2=box_sum(iv2,radius)
    cnt=np.where(cnt==0,1,cnt); mean=s1/cnt; var=s2/cnt-mean*mean
    return np.maximum(var,0.0)

def median_pred(img,parity):
    """median of REAL orthogonal neighbors only (all dot for a cross pixel), recoverable."""
...
def embed_layer(img,parity,T,payload_bits,start_bit,need,radius):
    ...
    out=img.copy(); bit_ptr=start_bit; end_bit=min(len(payload_bits),start_bit+need)
    exceptions=[]; N=0
    for idx in range(len(ii)):
        if bit_ptr>=end_bit: break
        i,j=ii[idx],jj[idx]; xh=x_hat[i,j]; e=int(img[i,j]-xh); N=idx+1
        if abs(e)<=T:
            b=int(payload_bits[bit_ptr]); xp=xh+2*e+b
            if 0<=xp<=255: out[i,j]=xp; bit_ptr+=1
            else: exceptions.append(idx)
        else:
            ep=e+(T+1) if e>T else e-(T+1); xp=xh+ep
            if 0<=xp<=255: out[i,j]=xp
            else: exceptions.append(idx)
    return out,dict(T=T,N=N,exceptions=exceptions,parity=parity,radius=radius),bit_ptr-start_bit
...
def run(cover,T,payload_bits,radius=2):
    ...
    s1,sc,e1=embed_layer(cover,0,T,payload_bits,0,half,radius)
    s2,sd,e2=embed_layer(s1,1,T,payload_bits,e1,P-e1,radius)
    return s2,(sc,sd),e1+e2

def decode(stego,sides):
    sc,sd=sides
    i1,bd=decode_layer(stego,sd); i0,bc=decode_layer(i1,sc)
    return i0,bc+bd

def experiment(path,name,payloads=(10000,20000,40000),Ts=(0,1,2,3,4,5,6),radius=2):
    cover=load_gray(path); rng=np.random.default_rng(12345)
    print(f"===== {name} =====")
    for P in payloads:
        best=None
        for T in Ts:
            ...
            if ber==0 and mse==0:
                p=psnr(cover,stego); nexc=len(sides[0]['exceptions'])+len(sides[1]['exceptions'])
                if best is None or p>best[1]: best=(T,p,nexc)
        print(f"  P={P:6d}: T={best[0]} PSNR={best[1]:.2f} dB exc={best[2]}" if best else f"  P={P:6d}: none")

if __name__=='__main__':
    for path,name in [('img/lena512.bmp','Lena'),('img/baboon.bmp','Baboon'),('img/boat512.bmp','Boat'),
